...
def main(args):
    ...
    parser.add_argument(
        "-n", "--num_images", default=10, type=int, help="num images to save"
    )
    parser.add_argument(
        "-d",
        "--directory",
    ...
    for i, (img, Type) in enumerate(ActualImages[0:max_images]):
        try:
            req = urllib.request.Request(str(img))
            with urllib.request.urlopen(req) as response:
                raw_img = response.read()
            #raw_img = urllib.request.urlopen(req).read()
            if len(Type) == 0:
                f = open(
                    os.path.join(save_directory, "img" + "_" + str(i) + ".jpg"), "wb"
                )

            else:
                f = open(
                    os.path.join(
                        save_directory, "img" + "_" + str(i) + "." + str(Type)
                    ),
                    "wb",
                )
            f.write(raw_img)
