...
def convert_subsection_to_text(subsection_file_path):
    ...
    class2 = gocr(subsection_file_path)
    return class1.process() , class2.process()
...
def create_image_subsections():
    ...
    lines = [line.rstrip('\n') for line in
             open('data/' + dir_name + '/res_img1.txt')]  # opening text file containing coordinates for boxes
    ...
    while i < len(coords):
        ...
        tess1 = tesseract(subsection_file_path)
        gocr1 = gocr(subsection_file_path)
        text1 = gocr1.process()
        text2 = tess1.process()
